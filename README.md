...
| `mcp_call`            | 调用MCP服务器执行命令                         | 可以访问各种集成的MCP工具和服务                            | 用户说："查询12306火车票信息" → `server_name: '12306-mcp'`    |
...
通过 `mcp_call` 工具函数调用MCP服务器：

```python
# 调用12306-mcp查询火车票信息
result = mcp_call(server_name="12306-mcp", command="query", args=["--from", "北京", "--to", "上海"])
```
